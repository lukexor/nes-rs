...
impl Memory {
    pub fn new() -> Self {
        let randomize = false;
        Self::with_capacity(0, randomize)
    }

    pub fn with_capacity(capacity: usize, randomize: bool) -> Self {
    ...
    pub fn from_bytes(bytes: &[Byte]) -> Self {
        let randomize = false;
        let mut memory = Self::with_capacity(bytes.len(), randomize);
    ...
    pub fn rom(capacity: usize) -> Self {
        let randomize = false;
        let mut rom = Self::with_capacity(capacity, randomize);
        rom.writable = false;
        rom
    }
    ...
    pub fn ram(capacity: usize, randomize: bool) -> Self {
        Self::with_capacity(capacity, randomize)
    }
    pub fn ram_from_bytes(bytes: &[Byte], randomize: bool) -> Self {
        let mut ram = Self::ram(bytes.len(), randomize);
...
impl BankedMemory {
    pub fn new(window: usize) -> Self {
        let randomize = false;
        Self::ram(0x2000, window, randomize)
    }

    pub fn ram(capacity: usize, window: usize, randomize: bool) -> Self {
        let memory = Memory::ram(capacity, randomize);
        Self {
            banks: Vec::new(),
            window,
...
mod tests {
    ...
    fn add_bank_range_test() {
        let randomize = false;
        let mut memory = BankedMemory::ram(0xFFFF, 0x2000, randomize);
        ...
        let mut memory = BankedMemory::ram(0xFFFF, 0x0400, randomize);
    ...
    fn add_bank_test() {
        let randomize = false;
        let mut memory = BankedMemory::ram(0xFFFF, 0x4000, randomize);
    ...
    fn peek_bank_test() {
        let randomize = false;
        let size = 40 * 1024;
        let rom = Memory::ram(size, randomize);
